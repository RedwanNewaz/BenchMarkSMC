...
//        int count = 0;
//        for (int i = 0; i < N; ++i) {
//            OBS_TYPE obs = history.Observation(i);
//            if(obs == BaseRockSample::E_BAD)
//                ++count;
//        }

//
//	    double phi_ = (N-count) /N;
////        cout<<"N "<<N <<" count "<<count<< " phi "<< phi_<< endl;
//
//
//        if(phi_<0.15)
//        value_= 10* State::Weight(particles)
//                     * Globals::Discount(grid_.xsize() - rs_model_->GetX(particles[0]) - 1);
//        else
            value_= 1* State::Weight(particles)
                    * Globals::Discount(grid_.xsize() - rs_model_->GetX(particles[0]) - 1);


        return ValuedAction(Compass::EAST, value_);
	}
};
...
public:
	...
	ValuedAction Value(const Belief* belief) const {
		...
		double value_;
		value_ = Globals::Discount(rs_model_->size_ - rs_model_->GetX(particles[0]) - 1) * 10;
